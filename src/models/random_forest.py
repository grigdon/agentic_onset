...
def evaluate_model(model, X_test, y_test, model_type_name):
    """
    Evaluates a trained model on the test set and returns performance metrics
    including AUC, predictions, feature importance, and classification report.

    Args:
        model (sklearn.pipeline.Pipeline): The trained model.
        X_test (pd.DataFrame): Test features.
        y_test (pd.Series): True test labels.
        model_type_name (str): A descriptive name for the model (e.g., "base_human").

    Returns:
        dict: A dictionary containing evaluation results.
    """
    if X_test.empty or y_test.empty or y_test.nunique() < 2:
        # Handle cases with insufficient data for evaluation
        return {
            'auc': 0.5,
